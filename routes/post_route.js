...
router.post("/createpost", protectedRoute, (req, res) => {
    ...
    if (!description || !date || !image) {
        return res.status(400).json({ error: "One or more mandatory fields are empty" });
    }
    req.user.password = undefined;
    const postObj = new PostModel({ description: description, date: date, image: image, author: req.user });
    postObj.save()
        .then((newPost) => {
...
router.put("/share", protectedRoute, (req, res) => {
    PostModel.findByIdAndUpdate(req.body.postId, {
        $push: { share: req.user._id }
    }, {
        new: true //returns updated record
    }).populate("author", "_id fullName")
        .exec((error, result) => {
